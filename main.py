...
def check_double_monster(ticker,typ,vol_k,strike,exp,price,opt_type,premium,row, above_ask=0):
    ...
    for combo,desc in combos:
        ...
        if not all(ticker in monster_memory[c] for c in combo):
            continue
        if max([monster_memory[c][ticker]["time"] for c in combo]) - min([monster_memory[c][ticker]["time"] for c in combo]) > 600:
            continue
        ...
        if ref["type"]=="C" and rsi>50:
            continue
        if ref["type"]=="P" and rsi<68:
            continue
        ...
        entry=ref["price"]
        if entry<=1.5:
            t1,t2,t3,stop = entry*2.0, entry*3.5, entry*6.0, entry*0.5
        else:
            t1,t2,t3,stop = entry*1.5, entry*2.0, entry*3.0, entry*0.7
                    ...
                    for exp in tk.options[:2]:
                        ...
                        for otype in ["calls","puts"]:
                            ...
                            for _,r in chain.iterrows():
                                ...
                                if not (0.25 <= price <= 12.0) or vol<30:
                                    continue
...
def main_loop():
    ...
    try:
        requests.post(f"https://api.telegram.org/bot{TOKEN}/sendMessage", data={"chat_id":CHAT_ID,"text":"🏆 <b>V25 FIXED الكامل شغال</b>","parse_mode":"HTML"}, timeout=15)
    except:
        pass
    ...
    while True:
        try:
            ...
            for u in res.get("result",[]):
                ...
                if "/test" in txt:
                    queue_send("🏆 V25 ✅ شغال KSA")
                if "/status" in txt:
                    queue_send(f"✅ ارسل اليوم {len(sent_today)} | طابور {len(message_queue)} | RSI {get_rsi('^GSPC'):.0f}\n⏰ {datetime.now(KSA).strftime('%H:%M KSA')}")
                if "/clear" in txt:
                    double_sent.clear()
                    sent_today.clear()
                    single_sent.clear()
