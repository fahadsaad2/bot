...
def check_double_monster(ticker,typ,vol_k,strike=0,exp="",price=0,opt_type="C",premium=0,row=None):
        ...
        if premium >= 500000:
            ...
            if single_key not in single_sent and single_key not in sent_today:
                ...
                if not ((opt_type=="C" and rsi>50) or (opt_type=="P" and rsi<68)):
                    if 0.5 <= price <= 12.0 and is_buy_signal(row or {}):
                        try:
    ...
    for combo,desc in combos:
        ...
        if not (0.5 <= ref["price"] <= 12.0): continue
        ...
        rsi=get_rsi("^GSPC" if "SPX" in ticker else ticker)
        opt_t=ref["type"]
        if opt_t=="C" and rsi>50:
            blocked_today+=1
            continue
        if opt_t=="P" and rsi<68:
            blocked_today+=1
            continue
        ...
        try:
            ...
            for key, mem in list(exit_memory.items()):
                ...
                try:
                    ...
                    r=row.iloc[0]; cur=float(r["lastPrice"]); vol=int(r["volume"])
                        ...
                        try:
                            ...
                            for otype in ["calls","puts"]:
                                ...
                                for _,r in chain.iterrows():
                                    ...
                                    if not (0.5 <= price <= 12.0): continue
                                    if vol < 30: continue
