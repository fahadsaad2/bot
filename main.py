...
def check_double_monster(ticker,typ,vol_k,strike=0,exp="",price=0,opt_type="C",premium=0):
    ...
    for combo,desc in combos:
        ...
        has_all = True
        for c in combo:
            if ticker not in monster_memory[c]:
                has_all=False
        if not has_all:
            continue
        ...
        try:
                            ...
                            for otype in ["calls","puts"]:
                                ...
                                for _,r in chain.iterrows():
                                    ...
                                    if vol / max(oi,1) > 1.2 and vol>100:
                                        monster_memory["SWEEPS"][dname]={"time":time.time(),"vol":prem_vol/1000,"strike":float(r['strike']),"exp":exp,"price":price,"type":otype_s,"premium":prem_vol}
                                    if ed==today_et and 0.10 <= price <= 6.0 and vol>50:
                                        key=f"HERO{sym}{r['strike']}{exp}{otype_s}"
                                        if is_new(key):
                                            msg=f"🚀 <b>HERO 0DTE {dname} {r['strike']:g}{otype_s}</b>\n📅 {exp}\n💵 ${price:.2f}"
                                    ...
                                    if (ed-today_et).days<=4 and 0.20 <= price <= 4.0 and vol>100:
                                        ...
                                        if is_new(key):
                                            msg=f"⚡ <b>POWER {dname} {r['strike']:g}{otype_s}</b>\n📅 {exp}\n💵 ${price:.2f}"
            ...
            if not all_found:
                if is_new("HEARTBEAT",h=0.25):
                    queue_send(f"💓 البوت شغال - فحص {scanned} عقد")
...
def main_loop():
    ...
    try:
        requests.post(f"https://api.telegram.org/bot{TOKEN}/sendMessage", data={"chat_id":CHAT_ID,"text":"✅ <b>V16.4 FIXED شغال - 192.5 مضبوط</b>","parse_mode":"HTML"}, timeout=15)
    except:
        pass
    ...
    while True:
        try:
            ...
            for u in r.get("result",[]):
                ...
                if "/test" in txt:
                    queue_send("🧪 V16.4 شغال ✅")
                if "/status" in txt:
                    queue_send(f"✅ LIVE طابور {len(message_queue)}")
                if "/clear" in txt:
                    sent.clear()
                    double_sent.clear()
                    message_queue.clear()
