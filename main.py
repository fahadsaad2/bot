                        ...
                        try:
                            ...
                            for otype in ["calls","puts"]:
                                ...
                                for _,r in chain.iterrows():
                                    vol=int(r['volume']) if r['volume'] else 0; price=float(r['lastPrice']) if r['lastPrice'] else 0
                                    ...
                                    if prem_vol > 2000000 and vol>300: check_double_monster(dname,"ULTRA",prem_vol/1000,float(r['strike']),exp,price,"C" if otype=="calls" else "P",prem_vol,r)
                                    elif prem_vol > 800000 and vol>300: check_double_monster(dname,"MEGA",prem_vol/1000,float(r['strike']),exp,price,"C" if otype=="calls" else "P",prem_vol,r)
                                    elif prem_oi>150000 and vol>200: check_double_monster(dname,"GOLDEN",prem_oi/1000,float(r['strike']),exp,price,"C" if otype=="calls" else "P",prem_oi,r)
                                    if vol / max(oi,1) > 3.0 and vol>500: check_double_monster(dname,"MOMENTUM",prem_vol/1000,float(r['strike']),exp,price,"C" if otype=="calls" else "P",prem_vol,r)
                                    if is_0dte and vol>50: check_double_monster(dname,"HERO",prem_vol/1000,float(r['strike']),exp,price,"C" if otype=="calls" else "P",prem_vol,r)
                        except: continue
                except: continue
            time.sleep(30)
        except Exception as e: print(f"ERR {e}"); time.sleep(10)
